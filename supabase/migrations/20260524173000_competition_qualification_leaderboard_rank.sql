-- Qualification standings use competition placement, so ties occupy the same
-- place and the next displayed place skips the tied slots.
...
as $$
  ...
  select
    ...
    rank() over (order by coalesce(points_totals.qualification_points, 0) desc)::integer as rank
  from guest_base
