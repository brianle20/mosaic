-- Qualification open-play sessions are untimed; keep the one-hour round limit
-- for tournament and bonus/finals sessions only.
  ...
  loop
    ...
    if not round_time_completed
      ...
      and hand_row.entered_at >=
        session_row.started_at + round_time_limit_duration then
      completion_flag := true;
