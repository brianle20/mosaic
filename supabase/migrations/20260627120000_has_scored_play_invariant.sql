create or replace function app_private.refresh_event_guest_scored_play(
...
begin
  with computed as (
    select
      ...
      or exists (
        ...
        join public.table_sessions as penalty_session
          on penalty_session.id = penalty.table_session_id
        where settlement.hand_result_id is null
          and penalty_session.event_id = guest.event_id
  ...
  from computed
  where guest.id = computed.event_guest_id
