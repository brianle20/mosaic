-- Mosaic MVP event activity feed
-- Checklist:
--   [x] add event-owned audit feed RPC
--   [x] map broad categories for host filters
--   [x] generate stable short summary text
--   [x] return newest-first rows
  ...
  scoped_logs as (
    select
      ...
      case
        ...
        when log.entity_type = 'event_table' and log.action = 'bind_tag' then
          format(
